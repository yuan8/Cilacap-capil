...
<script type="text/javascript">
  ...
  @foreach($target as $data)
...

    {
      ...
      url:"{{url('desa/'.$data->id)}}",
      location:null,
      windowContent:'<div id="content">'+
            '<div id="siteNotice">'+
            '</div>'+
            '<div id="bodyContent">'+
             '<h5 class="text-capitalize" >Desa :{{$data->nama}}</h5>'+
            '<h5 class="text-capitalize" > Kecamatan: {{$data->fromKecamatan->nama}}</h5>'+

            '<h5>{{$provinsi_count_capil}} Dapil</h5>'+
            '</div>'+
            '<a href="{{url('desa/'.$data->id)}}" class="btn btn-primary  btn-xs">Detail</a>'+
            '</div>',
      detail:{
        desa:"{{$data->nama}}",
        kecamatan:"{{$data->fromKecamatan->nama}}",
        kabupaten:"{{$data->fromKecamatan->fromKabupaten->nama}}",
        provinsi:"{{$data->fromKecamatan->fromKabupaten->fromProvinsi->nama}}"
      }
    },
  @endforeach
...
<div class="container-fluid">
  <div class="row">
    <div class="col-md-12" style="margin: 5px 0px 5px 0px;">
     <a href="{{url('kabupaten/'.$target_object->fromKabupaten->id)}}" class="btn btn-primary btn-xs"> < Back</a>
    </div>
  </div>
  <div class="row">
      ...
      <div class="row" style=" margin-top: 50px; overflow: scroll; height: 450px;">
        ...
        @foreach($target as $data)
          <div class="col-md-12" id="content-list-data-{{$data->id}}" style="border-bottom: 1px solid #f1f1f1; padding: 10px 15px;">
            <div class="row">
              <div class="col-md-8">
                <h5 class="text-uppercase">Desa : <span id="title-{{$data->id}}">{{$data->nama}}</span></h5>
                <h5 class="text-capilatize">Capil : <span id="capil-{{$data->id}}">0</span></h5>

              </div>
              <div class="col-md-4">
                <a href="{{url('desa/'.$data->id)}}" class="btn btn-warning btn-xs col-md-12" style="background: #efcf33">Detail</a>
              </div>
            </div>
...
<script type="text/javascript">
  ...
  function initialize() {
      ...
      for( var i in data){
        addMarker(i,(data[i].nama+','+data[i].detail.kecamatan+','+data[i].detail.kabupaten)); 
      }
